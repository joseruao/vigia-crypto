...
def _resolve_engine(provider: str, model_override: str | None = None):
    ...
    # DEVILS_ADVOCATE_PROVIDER (env) wins over the per-request value; default
    # keeps the historic behavior (openai).
    provider = (
        os.getenv("DEVILS_ADVOCATE_PROVIDER", "").strip().lower()
        or (provider or "openai")
    ).strip().lower()
    ...
    if env_base_url:
        ...
        if not azure_endpoint or not api_key:
            raise RuntimeError(
                "Motor Azure OpenAI não está configurado (faltam AZURE_OPENAI_ENDPOINT/AZURE_OPENAI_API_KEY)."
            )
        api_version = os.getenv("AZURE_OPENAI_API_VERSION", "2024-10-21")
        # No Azure OpenAI o "model" É o nome do deployment.
        model = os.getenv("DEVILS_ADVOCATE_AZURE_MODEL", os.getenv("AZURE_OPENAI_DEPLOYMENT", "gpt-5-mini"))
        is_local = False
    else:  # openai (default)
        base_url = None
